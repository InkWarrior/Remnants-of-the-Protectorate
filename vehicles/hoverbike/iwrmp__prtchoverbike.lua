...
function distanceToGround(point)
  ...
  -- get a list of pairs containing positions and liquid levels
  local liquidList = world.liquidAlongLine(point, endPoint)

  if intPoint then
    world.debugPoint(intPoint, {255, 255, 0, 255})
    return point[2] - intPoint[2]
  -- if liquidList is not empty (that is, if there is at least one position)
  elseif #liquidList > 0 then
	-- get the first pair (positions)
	local firstPair = liquidList[1]
	-- get the first position
	local firstPosition = firstPair[1]
	world.debugPoint(firstPosition, {255, 255, 0, 255})
	return point[2] - firstPosition[2]
  else
